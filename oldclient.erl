...
text_client(Socket) ->
    receive
        {put, K, V, Pid} ->
            Msg = io_lib:format("PUT ~p ~p", [K, V]);
        {del, K, Pid} ->
            Msg = io_lib:format("DEL ~p", [K]);
        {get, K, Pid} ->
            Msg = io_lib:format("GET ~p", [K]);
        {stats, Pid} ->
            Msg = io_lib:format("STATS")
    end,
